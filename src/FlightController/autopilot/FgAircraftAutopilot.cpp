...
FgAircraftAutopilot::FgAircraftAutopilot(FgControlledAircraft *aircraft, QObject *parent) : FgAutopilot(parent)
{
    m_Aircraft = aircraft;
...
void FgAircraftAutopilot::follow(FgAircraft *followAircraft)
{
    ...
    m_DesiredHeading = fgap::math::headingTo(m_Aircraft->latitude(), m_Aircraft->longitude(),
                                             followAircraft->latitude(), followAircraft->longitude());
